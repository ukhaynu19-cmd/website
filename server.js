...
const { Notice, Teacher, Application, SiteAdmin, Administration, TranslationCache } = require('./models');
...
async function translateBatch(texts, to) {
  ...
  // 1. Check DB cache first — shared across every visitor, not per-browser
  const cached = await TranslationCache.find({ text: { $in: nonEmptyTexts }, lang: to });
  const cacheMap = {};
  cached.forEach(c => { cacheMap[c.text] = c.translated; });

  const toFetch = [];
  const toFetchOrigIndex = [];
  nonEmptyTexts.forEach((t, i) => {
    if (cacheMap[t]) {
      results[nonEmptyIndices[i]] = cacheMap[t];
    } else {
      toFetch.push(t);
      toFetchOrigIndex.push(nonEmptyIndices[i]);
    }
  });

  if (toFetch.length === 0) return results;

  // 2. Call DeepL once, in a single batch request, for anything not cached
  if (!process.env.DEEPL_API_KEY) {
    console.error('DEEPL_API_KEY not set — skipping live translation for uncached text');
    toFetch.forEach((t, i) => { results[toFetchOrigIndex[i]] = t; }); // fallback: original text
    return results;
  }

  try {
    const params = new URLSearchParams();
    toFetch.forEach(t => params.append('text', t));
    ...
    if (!data.translations) {
      console.error('DeepL error:', JSON.stringify(data));
      toFetch.forEach((t, i) => { results[toFetchOrigIndex[i]] = t; });
      return results;
    }

    const toSave = [];
    data.translations.forEach((tr, i) => {
      results[toFetchOrigIndex[i]] = tr.text;
      toSave.push({
        updateOne: {
          filter: { text: toFetch[i], lang: to },
          update: { text: toFetch[i], lang: to, translated: tr.text },
          upsert: true
        }
      });
    });
    if (toSave.length > 0) await TranslationCache.bulkWrite(toSave);
  } catch (err) {
    console.error('DeepL translation failed:', err.message);
    toFetch.forEach((t, i) => { results[toFetchOrigIndex[i]] = t; }); // fallback to original text
  }
