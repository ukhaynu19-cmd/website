...
async function notifyApplicant(application, subject, message) {
  if (!application.email) return; // skip if no applicant email was provided
  try {
    await transporter.sendMail({
      from: process.env.NOTIFY_EMAIL_USER,
      to: application.email,
      subject: subject,
      text: `Dear ${application.guardianName},\n\n${message}\n\nStudent: ${application.studentName} (Class ${application.className})\n\n— Hill Academic Care`
    });
  } catch (err) {
  ...
  try {
    ...
    if (application) {
      notifyApplicant(application, 'Admission Confirmed — Hill Academic Care',
        'Congratulations! Your admission application has been accepted. Please contact the school office to complete the enrollment process.');
    }
    res.redirect('/admin/applications');
  } catch (err) {
    res.status(500).send('Database error');
